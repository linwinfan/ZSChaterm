...
function isStyleHidden(style: string): boolean {
  ...
  // clip-path: positive percentage inset() clipping hides content
  ...
  // transform: scale(0) or large negative translate
  ...
  if (width && /^\s*0(px)?\s*$/i.test(width[1]) && height && /^\s*0(px)?\s*$/i.test(height[1]) && overflow && /^\s*hidden\s*$/i.test(overflow[1])) {
    return true
  }
