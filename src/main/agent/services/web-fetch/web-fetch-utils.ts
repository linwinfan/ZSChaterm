...
export async function extractBasicHtmlContent(params: { html: string; extractMode: ExtractMode }): Promise<{ text: string; title?: string } | null> {
...
export async function extractReadableContent(params: {
  ...
  const cleanHtml = await sanitizeHtml(params.html)
  if (cleanHtml.length > READABILITY_MAX_HTML_CHARS || exceedsEstimatedHtmlNestingDepth(cleanHtml, READABILITY_MAX_ESTIMATED_NESTING_DEPTH)) {
    return null
  }
  try {
