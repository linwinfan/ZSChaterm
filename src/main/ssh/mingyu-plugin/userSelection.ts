...
import { type MingyuUser } from './parser'

/**
 * Handle Mingyu user selection with event sender
 ...
 */
export const handleMingyuUserSelectionWithEvent = (
...
/**
 * Handle Mingyu user selection with BrowserWindow
 * Used in contexts where we don't have an IPC event object
 * @param connectionId - Connection identifier
 * @param users - List of available users
 * @returns Promise that resolves with selected user ID
 */
export const handleMingyuUserSelectionWithWindow = (connectionId: string, users: MingyuUser[]): Promise<number> => {
  return new Promise<number>((resolve, reject) => {
    // Send user selection request to frontend
